...
@Component({
  ...
  standalone: false,
  templateUrl: './arena-breakout.component.html',
...
export class ArenaBreakoutComponent implements OnInit {
  ...
  guideImage = 'https://www.lnwtrue.com/_next/image?url=https%3A%2F%2Fmedia.lnwtrue.com%2Fimages%2Fuid%2Farena-breakout-pc%2FyZiYGBjILOs5K5.webp&w=1200&q=75';
  ...
  products = [
  { id: 'ab-0100', value: 100, amount: 39 },
  { id: 'ab-0500', value: 500, amount: 189 },
  { id: 'ab-1000', value: 1000, amount: 369 },
  { id: 'ab-2500', value: 2500, amount: 879 },
  { id: 'ab-5000', value: 5000, amount: 1729 },
  { id: 'ab-10000', value: 10000, amount: 3429 }
];
  ...
  ngOnInit(): void {
    const saved = localStorage.getItem('saved.uid.arenabreakout');
    if (saved) { this.uid = saved; this.validateUid(); }
  }
  saveUid(): void {
    if (!this.uid) return;
    localStorage.setItem('saved.uid.arenabreakout', this.uid);
    import('sweetalert2').then(({ default: Swal }) => Swal.fire({ icon: 'success', title: 'บันทึกแล้ว', text: 'บันทึก UID สำหรับครั้งถัดไปเรียบร้อย' }));
  }
  selectPayment(id: string): void { this.selectedPayment = id as any; }
  ...
  confirmPay(): void {
    ...
    setTimeout(() => {
      this.isPaying = false;
      this.isPaid = true;
      this.orderId = 'AB' + Math.floor(100000 + Math.random()*900000);
